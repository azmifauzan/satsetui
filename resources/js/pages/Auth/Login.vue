...
// Get status message from page props
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center px-4 sm:px-6 lg:px-8">
    <Head :title="currentLang === 'en' ? 'Sign In' : 'Masuk'" />
    ...
    <div class="max-w-md w-full">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white mb-2">
          {{ currentLang === 'en' ? 'Sign In to Your Account' : 'Masuk ke Akun Anda' }}
        </h1>
        <p class="text-slate-600 dark:text-slate-400">
          {{ currentLang === 'en' ? 'Welcome back! Please sign in to continue' : 'Selamat datang kembali!' }}
        </p>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8">
        ...
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex items-center justify-between">
            <label class="flex items-center">
              <input v-model="form.remember" type="checkbox" class="w-4 h-4 text-blue-600 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600 rounded focus:ring-blue-500" />
              <span class="ml-2 text-sm text-slate-600 dark:text-slate-400">{{ t.auth.rememberMe }}</span>
            </label>
          </div>
          ...
          <button type="submit" :disabled="form.processing"
            ...
            <span v-if="!form.processing">{{ currentLang === 'en' ? 'Sign In' : 'Masuk' }}</span>
        ...
        <div class="mt-6 text-center">
          <span class="text-slate-600 dark:text-slate-400">{{ currentLang === 'en' ? "Don't have an account?" : 'Belum punya akun?' }}</span>
          <Link :href="redirectTo ? `/register?redirect=${redirectTo}` : '/register'" class="ml-1 text-blue-600 dark:text-blue-400 hover:underline font-medium">
